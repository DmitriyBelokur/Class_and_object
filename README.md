...
Ок, после всей этой воды, мы уверено говорим что метод для получения размера стека, мы объявляем как статический. Ведт=ь нам надо вернуть константу размера стека, которую разделяют между собой все объекты этого класса.
